...
const CATEGORY_META: Record<Category, CategoryMeta> = {
  code: {
    ...
    glow: "shadow-[0_0_18px_-8px_rgba(139,92,246,0.55)]",
    icon: Code2,
    label: "code",
  },
  search: {
    border: "border-t-cyan-400/70",
    glow: "shadow-[0_0_18px_-8px_rgba(34,211,238,0.55)]",
    icon: Search,
    label: "search",
  },
  db: {
  ...
  http: {
    ...
    glow: "shadow-[0_0_18px_-8px_rgba(251,191,36,0.55)]",
    icon: Globe,
    label: "http",
  },
  shell: {
    border: "border-t-rose-400/70",
    glow: "shadow-[0_0_18px_-8px_rgba(251,113,133,0.55)]",
    icon: Terminal,
    label: "shell",
  },
  ai: {
    border: "border-t-fuchsia-400/70",
    glow: "shadow-[0_0_18px_-8px_rgba(232,121,249,0.55)]",
    icon: Sparkles,
    label: "ai",
  },
...
function classifyTool(name: string): Category {
  ...
  if (/(write_file|line_replace|edit|patch|apply_diff|create_file|delete_file|format)/.test(n))
    return "code";
...
const STATUS_META: Record<
  ToolCallStatus,
  { label: string; ring: string; dot: string; icon: typeof CircleDot }
> = {
  queued: {
    label: "queued",
...
export default function ToolCallBubble({ tool, onAbort }: ToolCallBubbleProps) {
  ...
  async function handleAbort(e: React.MouseEvent) {
    ...
    setAborting(true);
    try {
      await onAbort(tool.id, tool.abort_token);
    } finally {
      setAborting(false);
    }
  }
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="font-mono text-[9px] uppercase tracking-wider text-muted-foreground/45">
          ·{catMeta.label}
        </span>
        <StatusIcon
          className={`h-3 w-3 shrink-0 ${statusMeta.dot} ${spinning ? "animate-spin" : ""}`}
        />
        <span className={`font-mono text-[9px] uppercase tracking-wider ${statusMeta.dot}`}>
          {statusMeta.label}
        </span>
            ...
            <span
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter" || e.key === " ")
                  handleAbort(e as unknown as React.MouseEvent);
              }}
      ...
      {isRunning && (
        <div className="relative h-[3px] w-full overflow-hidden bg-white/[0.04]">
          {hasProgress ? (
            ...
            <div
              className={`absolute inset-y-0 w-1/3 ${catMeta.border.replace("border-t-", "bg-")} tc-shimmer opacity-80`}
            />
          )}
        </div>
      )}
      ...
      {open && (
        <div className="space-y-2 border-t border-white/[0.04] bg-black/20 px-2.5 py-2">
          {tool.args !== undefined && (
            <div>
              <div className="mb-1 font-mono text-[9px] uppercase tracking-wider text-muted-foreground/50">
                Input
              </div>
          ...
          {tool.error && (
            <div>
              <div className="mb-1 font-mono text-[9px] uppercase tracking-wider text-red-400/70">
                Error
              </div>

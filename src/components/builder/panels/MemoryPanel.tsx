...
const AGENTS: { slug: AgentSlug; name: string }[] = [
  { slug: "jimmy", name: "Jimmy" }, { slug: "sherlock", name: "Sherlock" },
  { slug: "aria", name: "Aria" }, { slug: "orion", name: "Orion" }, { slug: "rex", name: "Rex" },
  { slug: "lyra", name: "Lyra" }, { slug: "sage", name: "Sage" }, { slug: "atlas", name: "Atlas" },
  { slug: "vega", name: "Vega" }, { slug: "kai", name: "Kai" },
];

const SCOPE_TONE: Record<string, "emerald" | "violet" | "sky" | "amber" | "gray"> = {
  episodic: "sky", semantic: "violet", procedural: "amber", fact: "emerald",
};
...
export default function MemoryPanel() {
  ...
  return (
    <div>
      <PanelSection title="Agent" action={<span className="text-[10px] text-muted-foreground/60">{AGENTS.length}</span>}>
        <div className="flex flex-col gap-0.5">
          {AGENTS.map((a) => (
            <Row
              ...
              left={<><Dot tone={selected === a.slug ? "emerald" : "gray"} /><span>{a.name}</span></>}
              right={a.slug}
            />
          ))}
        </div>
      ...
      <PanelSection title="Memories" action={<span className="text-[10px] text-muted-foreground/60">{rows.length}</span>}>
        {loading ? (
          ...
          <div className="flex flex-col gap-1">
            {rows.map((m) => <MemoryRow key={m.id} m={m} />)}
          </div>
        )}
      </PanelSection>
    </div>
    ...
    <div className="rounded-md border border-white/[0.05] bg-white/[0.01] px-2 py-1.5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[10px] uppercase tracking-wider text-muted-foreground/80">{m.scope}</span>

...
function computeMetrics(activity: AgentActivity[]): Metric[] {
  ...
  const avgLat = withDur.length
    ? withDur.reduce((s, a) => s + (a.duration_ms ?? 0), 0) / withDur.length
    : null;
  ...
  return [
    ...
    {
      label: "Avg latency",
      value: avgLat ? `${Math.round(avgLat)}ms` : "—",
      tone: avgLat && avgLat > 4000 ? "amber" : "gray",
    },
...
export default function AnalyticsPanel() {
  ...
  useEffect(() => {
    ...
    Promise.allSettled([listActivity({ projectId: project, limit: 500 }), getBridgeHealth(project)])

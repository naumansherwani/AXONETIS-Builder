...
export async function callHostFlowServer<TResponse>(
  path: string,
  init?: RequestInit,
): Promise<TResponse> {
...
export type RapidPayAgentSlug =
  ...
  | "settlement-hawk"
  | "fraud-radar"
  | "treasury-stress-intelligence"
  | "revenue-brain"
  | "explainability-civilization"
  | "founder-sandbox-civilization"
  | "global-router";
    ...
    for (const frame of frames) {
      ...
      if (event === "ack") {
        ...
        handlers.onDone?.({
          ...
          assistantText:
            typeof payload.assistantText === "string" ? payload.assistantText : undefined,
          status: typeof payload.status === "string" ? payload.status : undefined,
        });
      } else if (event === "error") {
        handlers.onError?.(
          typeof payload.error === "string" ? payload.error : "Unknown stream error",
        );
      }
    }
  }
...
export async function cancelAgentStream(streamId: string) {
  return callHostFlowServer<{ ok: boolean; status?: string }>(
    `/api/agents/stream/${streamId}/cancel`,
    {
      method: "POST",
    },
  );
...
export function listActivity(
  params: { projectId?: ProjectId; agentSlug?: AgentSlug; limit?: number } = {},
) {
  const q = new URLSearchParams();
  if (params.projectId) q.set("projectId", params.projectId);
  if (params.agentSlug) q.set("agentSlug", params.agentSlug);
...
export function subscribeActivity(
  ...
  es.onmessage = (ev) => {
    try {
      onEvent(JSON.parse(ev.data) as AgentActivity);
    } catch {
      /* ignore */
    }
  };
...
// Router
export function routeTask(body: {
  projectId: ProjectId;
  task: string;
...
export const listRapidPayAgents = () =>
  callHostFlowServer<RapidPayAgentInfo[]>("/api/rapidpay/agents");
...
export function runRapidPayFraudScan(projectId: ProjectId, target?: string) {
  return callHostFlowServer<{ scanId: string; status: string }>(
    "/api/rapidpay/agents/fraud-radar/scan",
    {
      method: "POST",
      body: JSON.stringify({ projectId, target }),
    },
...
export function runRapidPayTreasurySentinelScan(projectId: ProjectId, target?: string) {
  return callHostFlowServer<{ scanId: string; status: string }>(
    "/api/rapidpay/agents/treasury-sentinel/scan",
    {
      method: "POST",
      body: JSON.stringify({ projectId, target }),
    },
...
export function runRapidPaySherlockSecurityScan(projectId: ProjectId, target?: string) {
  return callHostFlowServer<{ scanId: string; status: string }>(
    "/api/rapidpay/security/sherlock/scan",
    {
      method: "POST",
      body: JSON.stringify({ projectId, target }),
    },
...
export function listRapidPayThreads(
  params: { projectId?: ProjectId; agentSlug?: RapidPayAgentSlug } = {},
) {
  const q = new URLSearchParams();
  if (params.projectId) q.set("projectId", params.projectId);
  if (params.agentSlug) q.set("agentSlug", params.agentSlug);
...
export function getRapidPayAgentMemory(
  slug: RapidPayAgentSlug,
  ...
  if (params.limit) q.set("limit", String(params.limit));
  const qs = q.toString();
  return callHostFlowServer<AgentMemoryRow[]>(
    `/api/rapidpay/agents/${slug}/memory${qs ? `?${qs}` : ""}`,
  );
}

export function writeRapidPayAgentMemory(
  slug: RapidPayAgentSlug,
  body: {
    scope: "episodic" | "semantic" | "procedural" | "fact";
    content: string;
...
export function listRapidPayActivity(
  params: { projectId?: ProjectId; agentSlug?: RapidPayAgentSlug; limit?: number } = {},
) {
  const q = new URLSearchParams();
  if (params.projectId) q.set("projectId", params.projectId);
  if (params.agentSlug) q.set("agentSlug", params.agentSlug);
...
export function routeRapidPayTask(body: {
  projectId: ProjectId;
  task: string;
  context?: Record<string, unknown>;
}) {
...
export function dispatchRapidPaySwarm(body: {
  projectId: ProjectId;
  task: string;
  agents?: RapidPayAgentSlug[];
  context?: Record<string, unknown>;
}) {

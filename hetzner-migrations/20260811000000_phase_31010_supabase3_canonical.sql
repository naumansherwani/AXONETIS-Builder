...
-- Idempotent + self-healing: legacy/incompatible tables of the same name are
-- quarantined (renamed to *_legacy_<epoch>) instead of failing the migration.

create extension if not exists vector;
create extension if not exists pg_trgm;

/* ───────── 0. Quarantine incompatible legacy tables ─────────
   An earlier install created public.tool_call_registry with a totally
   different shape (no id column). Views on it are dropped first, then the
   table is renamed aside so the canonical DDL below can create cleanly. */

drop view if exists public.tool_calls cascade;
drop view if exists public.tool_cost_daily cascade;

do $$
declare
  stamp text := to_char(now(), 'YYYYMMDDHH24MISS');
begin
  -- tool_call_registry must have id + tool_name + started_at/created_at
  if exists (select 1 from information_schema.tables
              where table_schema = 'public' and table_name = 'tool_call_registry')
     and not exists (select 1 from information_schema.columns
                      where table_schema = 'public' and table_name = 'tool_call_registry'
                        and column_name = 'id') then
    execute format('alter table public.tool_call_registry rename to tool_call_registry_legacy_%s', stamp);
  end if;
  ...
  if exists (select 1 from information_schema.tables
     ...
     and not exists (select 1 from information_schema.columns
                      where table_schema = 'public' and table_name = 'agent_subagents'
                        and column_name = 'id') then
    execute format('alter table public.agent_subagents rename to agent_subagents_legacy_%s', stamp);
  end if;

  if exists (select 1 from information_schema.tables
              where table_schema = 'public' and table_name = 'mem_entries')
     and not exists (select 1 from information_schema.columns
                      where table_schema = 'public' and table_name = 'mem_entries'
                        and column_name = 'content') then
    execute format('alter table public.mem_entries rename to mem_entries_legacy_%s', stamp);
  end if;
end $$;

/* ───────── 1. tool_call_registry (canonical) ───────── */
...
-- additive columns for partial older installs (never drop)
...
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'tool_call_registry_status_chk') then
    alter table public.tool_call_registry
      add constraint tool_call_registry_status_chk
      check (status in ('running','ok','error','aborted')) not valid;
  end if;
end $$;

create index if not exists tool_call_registry_thread_idx  on public.tool_call_registry (thread_id, started_at desc);
create index if not exists tool_call_registry_project_idx on public.tool_call_registry (project_id, started_at desc);
create index if not exists tool_call_registry_live_idx    on public.tool_call_registry (status) where status = 'running';
create index if not exists tool_call_registry_tool_idx    on public.tool_call_registry (tool_name, started_at desc);
...
create policy "tool_call_registry readable" on public.tool_call_registry for select using (true);

-- Blueprint alias view (no duplicate table)
create view public.tool_calls as
  select id, agent_slug as agent_id, tool_name, input, output, cost, status,
         started_at as created_at, thread_id, project_id, duration_ms
  from public.tool_call_registry;
grant select on public.tool_calls to anon, authenticated, service_role;

-- Cost Meter rollup (3.9.7)
create view public.tool_cost_daily as
  ...
  from public.tool_call_registry group by 1, 2;
...
alter table public.agent_subagents add column if not exists task text;
...
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'agent_subagents_status_chk') then
    alter table public.agent_subagents
      add constraint agent_subagents_status_chk
      check (status in ('queued','running','done','failed','cancelled')) not valid;
  end if;
end $$;

create index if not exists agent_subagents_thread_idx on public.agent_subagents (thread_id, created_at desc);
create index if not exists agent_subagents_parent_idx on public.agent_subagents (parent_id);
create index if not exists agent_subagents_live_idx   on public.agent_subagents (thread_id) where status in ('queued','running');
...
create policy "agent_subagents readable" on public.agent_subagents for select using (true);

-- Runaway swarm guard: max 5 live per thread, max depth 3
...
begin
  ...
  if new.status in ('queued','running') then
    select count(*) into live_count from public.agent_subagents
     where thread_id = new.thread_id and status in ('queued','running') and id <> new.id;
...
/* ───────── 3. mem_entries (pgvector 1536 = openai/text-embedding-3-small) ───────── */
...
do $$ begin
  if not exists (select 1 from pg_constraint where conname = 'mem_entries_scope_chk') then
    alter table public.mem_entries
      add constraint mem_entries_scope_chk
      check (scope in ('episodic','semantic','procedural','fact','decision')) not valid;
  end if;
end $$;

create unique index if not exists mem_entries_dedupe_idx
  on public.mem_entries (agent_id, coalesce(project_id,''), content_hash);
create index if not exists mem_entries_agent_idx     on public.mem_entries (agent_id, scope, importance desc);
create index if not exists mem_entries_project_idx   on public.mem_entries (project_id, created_at desc);
create index if not exists mem_entries_trgm_idx      on public.mem_entries using gin (content gin_trgm_ops);
create index if not exists mem_entries_embedding_idx on public.mem_entries using hnsw (embedding vector_cosine_ops);
...
create policy "mem_entries admin read" on public.mem_entries for select to authenticated using (true);
...
create or replace function public.match_mem_entries(
  ...
  id uuid, agent_id text, project_id text, scope text, title text, content text,
  importance numeric, similarity double precision, created_at timestamptz
...
$$;
grant execute on function public.match_mem_entries(vector, text, text, integer, double precision) to authenticated, service_role;

create or replace function public.search_mem_entries(
  q text, p_agent_id text default null, match_count integer default 8
) returns setof public.mem_entries
language sql stable security definer set search_path = public as $$
  select * from public.mem_entries
...
begin
  ...
  where pinned = false and importance < 0.35 and access_count = 0

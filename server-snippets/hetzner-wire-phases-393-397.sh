...
select_db_url() {
  ...
  die "Local Postgres peer access nahi mila. Root se run karo ya AXONETIS_DB_URL actual direct DB URL do."
}
...
run_psql() {
  ...
  if [ "$code" -ne 0 ]; then
    if [[ "$DB_URL" != local-peer:* ]] && printf '%s' "$PSQL_OUT" | grep -qi "password authentication failed"; then
      ...
      if [ -n "$peer_url" ]; then
        warn "TCP password failed; switching to local Postgres peer DB (${peer_url#local-peer:}). No DB password needed."
        DB_URL="$peer_url"
        set +e
        local peer_db="${DB_URL#local-peer:}"
        if [ "$mode" = "file" ]; then
